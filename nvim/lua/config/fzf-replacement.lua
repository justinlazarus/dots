-- Complete fzf-lua replacement for Snacks picker
-- This gives you ALL the same functionality with proper input handling
...
return {
  ...
  config = function()
    ...
    fzf.setup({
      'default-title', -- Use default configuration with title
      winopts = {
        height = 0.85,
        width = 0.80,
        row = 0.35,
      ...
      keymap = {
        -- Proper input handling - no backspace issues!
        fzf = {
          ["ctrl-q"] = "select-all+accept",
          ["ctrl-u"] = "unix-line-discard", -- Clear entire line (works perfectly!)
          ["ctrl-w"] = "unix-word-rubout",  -- Delete word backwards
        },
      },
      -- Git integration
      git = {
      ...
      lsp = {
        prompt_postfix = '❯ ',
        symbols = {
          symbol_style = 1,     -- Show symbol kind
  ...
  keys = {
    -- EXACT same keybindings you have now!
    ...
    { '<leader>n', function() 
      -- Notifications - can use quickfix or custom implementation
      vim.cmd('messages') 
    end, desc = 'Notification History' },
    { '<leader>e', '<cmd>FzfLua files<cr>', desc = 'File Explorer' }, -- or use Oil/NvimTree
